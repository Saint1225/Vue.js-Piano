...
for (i=0;i<soundindex.length;i++){
  soundpack.push({
    number: soundindex[i],
    url: "https://awiclass.monoame.com/pianosound/set/" + soundindex[i] +".wav"
    }
  )
};
...
$(window).keydown(function(e){
  ...
  for (i=0;i<vm.display_keys.length;i++){
    if (key==vm.display_keys[i].key){
      vm.addnote(vm.display_keys[i].num,0.7);
    }
  }
})
